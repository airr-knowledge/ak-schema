...
sqlddl: src/ak_schema/schema/ak_top_sqlddl.yaml
	...
	tail --lines=+2 $(SQL_DDL_PATH).txt | sed -e 's/DATETIME/TIMESTAMP WITHOUT TIME ZONE/g' | sed -e 's/"Investigation_akc_id"/investigation_akc_id/g' | sed -e 's/"Antigen_akc_id"/antigen_akc_id/g' > $(SQL_DDL_PATH)
